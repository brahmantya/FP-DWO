...
//QUERY CHART PERTAMA
$sql = "SELECT count(customer_id) as tot from fakta_pendapatan";
$tot = mysqli_query($mysqli,$sql);
$tot_amount = mysqli_fetch_row($tot);

$sql = "SELECT concat('name:',s.nama_kota) as name, concat('y:', count(fp.customer_id)*100/" . $tot_amount[0] . ") as y, concat('drilldown:' , s.nama_kota) as drilldown
        FROM store s
...
$sql = "SELECT s.nama_kota kategori, count(fp.customer_id) as tot_kat
...
$sql = "SELECT s.nama_kota kategori, 
        t.bulan AS bulan,
        count(fp.customer_id) AS pendapatan_kat
        FROM store s
        JOIN fakta_pendapatan fp ON (s.store_id = fp.store_id)
...
foreach ($data_det[0] as $a) {
    if($i < count($data_det[0])-1){
        if($a[0] != $data_det[0][$i+1][0]){
            ...
            $string_data .= '{name:"Percentage of Customer Numbers by month in ' .$a[0] . '", id:"' . $a[0] . '", data: [';
        } else {
            $string_data .= '[' . $a[1] . ', '.
                $a[2]*100/cari_tot_kat($a[0], $tot_all_kat) . '], ';
        }
            ...
            <div class="sidebar-inner slimscroll">
                <div id="sidebar-menu" class="sidebar-menu">
                    <ul>
                        ...
                        <li class="submenu">
                            <a href="index.php"><i class="fa fa-home"></i> <span> Dashboard </span> <span class="menu-arrow"></span></a>
...
<script type="text/javascript">
    Highcharts.chart('container',{
        ...
        title: {
            text: 'Presentase Nilai Penjualan (WH Sakila) Di Semua Toko - Semua Kategori'
        },
        subtitle: {
        ...
        drilldown: {
            ...
            <?php
                    echo $string_data;
                ?>
                //{name:"Action", id:"Action", data: [["2", 1.2751808220117], ["5"], 8.4813236285522], ["6", 15.571831758401]}
            ]
        }
    });
</script>
